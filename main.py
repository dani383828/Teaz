...
async def get_user_subscriptions(user_id):
    try:
        ...
        rows = await db_execute(
            "SELECT id, plan, config, status, payment_id, start_date, end_date FROM subscriptions WHERE user_id = %s ORDER BY end_date DESC",
            (user_id,), fetch=True)
        return rows
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("⚠️ خطایی رخ داده است. لطفا دوباره امتحان کنید.")
...
async def contact_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("⚠️ خطایی در ثبت شماره تماس رخ داد. لطفا دوباره امتحان کنید.")
...
async def start_with_param(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logging.error(f"Error in start_with_param: {e}")
        await update.message.reply_text("⚠️ خطایی رخ داده است. لطفا دوباره امتحان کنید.")
...
async def on_startup():
    try:
        ...
        logging.info(f"✅ Bot started successfully. Webhook: {WEBHOOK_URL}")
    except Exception as e:
