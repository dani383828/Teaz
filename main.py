...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # بررسی وجود کاربر در دیتابیس (یعنی قبلا شماره داده)
    cursor.execute("SELECT user_id FROM users WHERE user_id=?", (user_id,))
    user_exists = cursor.fetchone() is not None

    # اگر کاربر قبلا ثبت شده، مستقیم به منوی اصلی برو
    if user_exists:
        await update.message.reply_text(
...
async def contact_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await application.bot.send_message(
        chat_id=ADMIN_ID,
        text=f"📞 کاربر {user_id} شماره تماس خود را ارسال کرد: {contact.phone_number}"
    )
    ...
    if update.message.photo or update.message.document:
        ...
        if state and (state.startswith("awaiting_deposit_receipt_") or state.startswith("awaiting_subscription_receipt_")):
            ...
            try:
                ...
                await update.message.reply_text("❌ خطا در ارسال فیش به ادمین. لطفا با پشتیبانی تماس بگیرید.", reply_markup=get_main_keyboard())
...
application.add_handler(MessageHandler(filters.CONT четвер

System: It looks like the code you provided was cut off at the end. I'll complete the handler registration part and ensure the three specified issues are addressed in the full code. The issues fixed are:

1. Sending the user's contact number to the admin's numeric ID in `contact_handler`.
2. Checking if the user has already provided their contact and is a channel member in `start` to avoid repeated prompts.
3. Ensuring payment receipts (photo or document) are sent to the admin’s numeric ID in `message_handler` with error handling.

Below is the complete corrected code with only the specified changes and the handler registration completed:
