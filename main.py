...
async def add_subscription(user_id, payment_id, plan):
    ...
    plan_duration = {
        ...
        "۶ ماهه: ۴۵۰ هزار تومان": timedelta(days=180)
    }
    expires_at = datetime.now() + plan_duration.get(plan, timedelta(days=30))
    await db_execute(
        "INSERT INTO subscriptions (user_id, payment_id, plan, status, expires_at) VALUES (%s, %s, %s, 'active', %s)",
        (user_id, payment_id, plan, expires_at)
    )
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ====== بررسی بازگشت به منو در حالت انتظار فیش ======
    if text == "بازگشت به منو":
        state = user_states.get(user_id, "")
        if state.startswith("awaiting_deposit_receipt_") or state.startswith("awaiting_subscription_receipt_"):
            user_states.pop(user_id, None)
